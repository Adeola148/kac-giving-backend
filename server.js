...
// Root + health
app.get('/', (_req, res) => res.send('KAC Giving Backend is running ✅'));
...
app.post('/create-checkout', async (req, res) => {
  try {
    ...
    const s = getStripe();
    if (!s) return res.status(500).json({ error: 'Stripe not configured on server (missing STRIPE_SECRET_KEY).' });
